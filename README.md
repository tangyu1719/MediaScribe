...
收藏同步复用用户已授权登录的 CDP Chrome。页面加载前会注入一次性 XHR/fetch hook，读取小红书页面自己已经签名并请求成功的 `/api/sns/web/v2/note/collect/page` 响应，再与原有 `__INITIAL_STATE__`、DOM href 和逐条点击补链路合并去重。

这条路径不在浏览器外计算或破解 `X-S`、`X-T`、`X-S-Common`，也不会把请求头、Cookie 或签名值传给后端。实现思路参考 MIT 项目 [PCPrincipal67/xhs-favorites-exporter](https://github.com/PCPrincipal67/xhs-favorites-exporter)，只提取了最小响应捕获机制，没有引入扩展 UI。

采集优先级与边界：

1. 页面内收藏分页响应：补齐 SSR/DOM 没有暴露的分页笔记；
2. `__INITIAL_STATE__` 与 DOM：保留为首屏和页面结构回退；
3. 点击补链接：仅对仍缺少有效访问 URL 的笔记执行；
4. 响应捕获失败或接口结构变化时自动回落到旧路径，不阻断手动同步；
5. 收藏同步保持手动触发，应用启动和定时调度不会自动执行收藏夹订阅任务。

同参数 A/B 诊断只输出数量、字段完整率、来源分布和耗时，不输出笔记 URL 或 token：

```powershell
py -3.12 deploy\compare_xhs_favorites_capture.py --scroll-rounds 4
```

```bash
python3 deploy/compare_xhs_favorites_capture.py --scroll-rounds 4
```

脚本会从已打开且已登录的小红书页面自动识别当前账号；也可显式传入 `--creator-id`。设置 `SBA_XHS_FAVORITES_RESPONSE_CAPTURE=0` 可单独关闭新路径，用于排障或回归旧行为。

2026-07-31 在同一登录账号、相同 4 轮滚动参数下的实测：

| 指标 | 旧 SSR/DOM 路径 | 响应捕获 + 旧路径回退 |
|------|-----------------|------------------------|
| 收藏笔记数 | 10 | 30 |
| note_id 完整率 | 100% | 100% |
| xsec_token 完整率 | 100% | 100% |
| 标题完整率 | 100% | 100% |
| 作者完整率 | 100% | 100% |
| 抓取耗时 | 5.77 秒 | 10.21 秒 |

本次账号的首屏 DOM 只暴露 10 条；页面内捕获到 2 页 XHR、额外恢复 20 条。新增路径多一次首屏刷新，约增加 4.44 秒，但将本次覆盖量提升 200%。结果会随账号收藏量、网络和小红书页面结构变化，应在升级后用上述脚本复测。
