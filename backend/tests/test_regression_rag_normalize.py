...
class TestDocNormalizer:
    ...
    def test_normalize_md_download_link_describe_reinsert(self, sample_md_with_local_image: Path):
        ...
        assert "/output/kb_assets/" in r.text
        assert "![登录页]" not in r.text or "screen.png" not in r.text
...
class TestKbChunkAfterNormalize:
    def test_split_after_normalize(self, sample_md: Path):
        from app.services.doc_normalizer import normalize_document
        from app.services.chonkie_chunker import chunk_text_with_meta

        r = normalize_document(sample_md, tenant_id=1, doc_id=104)
        stats = chunk_text_with_meta(r.text, mode="md_header", source="faq.md")
        chunks = stats.get("chunks") or []
        assert len(chunks) >= 1


class TestSbaKbOutput:
    def test_public_url(self, tmp_path: Path, monkeypatch):
        from app.services import sba_kb_output as ho
        from app.services.task_manager import get_output_dir
