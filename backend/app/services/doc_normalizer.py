...
from app.services.sba_kb_output import kb_assets_dir
...
def _process_images_inline(
    ...
    *,
    ...
    for i, src in enumerate(image_paths[:limit], start=1):
        img_id = f"img_{i:04d}"
        dest = asset_root / "images" / f"{img_id}{src.suffix.lower() or '.png'}"
        if src.resolve() != dest.resolve():
            shutil.copy2(src, dest)
        res = process_image(
            str(dest),
...
def _normalize_docx_p0(
    ...
    *,
    ...
    final_parts: List[str] = [f"# {source.stem}\n"]
    img_results: List[ImageProcessResult] = []
    img_cursor = 0
    truncated = False
    images_dir = asset_root / "images"
    ...
    for block in parsed.blocks:
        if block.kind == "paragraph":
            ...
            res = process_image(
                str(dest),
                image_id=f"img_{img_cursor:04d}",
                doc_context="\n".join(context_acc)[-2000:],
                source_format="docx",
            )
            img_results.append(res)
            final_parts.append(res.rag_block)
...
def _normalize_docx(
    ...
    *,
    ...
    p0 = _normalize_docx_p0(source, asset_root, limit=limit, inspect=inspect)
    if p0 is not None:
        return p0
    ...
    *,
    ...
    try:
        ...
        if suffix in {".docx", ".doc"}:
            return _normalize_docx(source, asset_root, limit=limit, inspect=inspect)

        if suffix == ".pdf":
            return _normalize_pdf(source, asset_root, limit=limit, inspect=inspect)
        ...
        if suffix in {".xlsx", ".xls", ".pptx", ".ppt"}:
            return _normalize_office_other(source, asset_root, limit=limit, inspect=inspect)
